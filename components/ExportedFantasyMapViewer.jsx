...
export default function ExportedFantasyMapViewer({ mapImage, locations = [], regions = [] }) {
  ...
  const locationsByRegion = regions.map(r => ({
    ...
    locations: locations.filter(l => {
      ...
      return pointInPolygon(xPx, yPx, r.pointsPercent, imgSize)
    })
  }))
  ...
  const unassignedLocations = locations.filter(l => {
    const xPx = (l.x / 100) * imgSize.width
    const yPx = (l.y / 100) * imgSize.height
    return !regions.some(r => pointInPolygon(xPx, yPx, r.pointsPercent,imgSize))
  })

  return (
    <div className="relative bg-gray-900/20 rounded-lg p-2 overflow-hidden shadow-inner w-full">
      {/* Legend Toggle */}
      ...
      <TransformWrapper initialScale={1} minScale={0.5} maxScale={4} ref={transformRef}>
        <TransformComponent>
          <div className="relative w-full max-w-full mx-auto">
            <img
              ...
              onLoad={onImgLoad}
              alt="map"
              className="w-full h-auto rounded-md select-none shadow-lg"
            />
            ...
            {locations.map(loc => (
              ...
              >
                <div
                  ...
                  className="w-4 h-4 sm:w-5 sm:h-5 bg-red-500 rounded-full border-2 border-white cursor-pointer shadow-lg hover:scale-125 transition-transform"
                />
              </div>
            ))}
            ...
            {hoverItem && (
              <div
                ...
                className="bg-black/70 text-white px-2 py-1 rounded text-xs shadow-lg pointer-events-none z-50 max-w-[150px] text-center"
              >
                {hoverItem.name}
              </div>
            )}
      ...
      {/* Legend */}
      {legendVisible && (
        <div className="absolute top-14 left-2 z-40 bg-black/50 backdrop-blur-md p-3 rounded-lg shadow-md max-h-64 overflow-auto text-sm text-white w-40 sm:w-60 md:w-72">
        ...
        {showModal && modalItem && (
          ...
          >
            ...
            <motion.div
              ...
              className="relative bg-gray-900/80 backdrop-blur-xl border border-indigo-500/30 p-4 sm:p-6 rounded-3xl shadow-2xl w-full max-w-md max-h-[80vh] overflow-y-auto overflow-x-hidden"
            >
              <div className="absolute -top-6 left-1/2 transform -translate-x-1/2 w-16 h-1 rounded-full bg-indigo-400/50 animate-pulse"></div>

              <h3 className="font-extrabold text-xl sm:text-2xl text-indigo-400 mb-3 text-center drop-shadow-md">
                {modalItem.name}
              </h3>
              ...
              <div className="flex justify-center gap-2 sm:gap-3 flex-wrap">
                {modalItem.lore?.length > 140 && (
                  <motion.button
                    ...
                    className="px-3 sm:px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold transition-shadow shadow-md"
                  >
                ...
                {modalItem.link && (
                  <motion.a
                    ...
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    className="px-3 sm:px-4 py-2 rounded-xl bg-green-600 hover:bg-green-500 text-white font-semibold transition-shadow shadow-md"
                  >
                    Open Link
                  </motion.a>
                )}
                ...
                <motion.button
                  ...
                  className="px-3 sm:px-4 py-2 rounded-xl bg-gray-700 hover:bg-gray-600 text-white font-semibold transition-shadow shadow-md"
                >
                  Close
                </motion.button>
              </div>
